	...
	public class InformationClass
	{
		...
		DateTime GetOSInstallDate()
		{
			...
			try
			{
				...
				if (DateTime.TryParse("01/01/1970 00:00:00", out origDate))
				{
		...
		static string GetOSType()
		{
			...
			{
				rk = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Microsoft\Windows NT\CurrentVersion");
				string type = rk.GetValue("CurrentType", "Not Found").ToString();
				rk.Close();
				return type;
			}
			catch
		...
		static string GetOSProductKey()
		{
			try
			{
				...
				if (digitalPid != null)
				{
					...
					var key = new byte[15]; //0-14 = 15 bytes
					...
					for (int j = 0; j <= 24; j++)
					{
						...
						for (int i = 14; i >= 0; i--)
						{
							...
							key[i] = Convert.ToByte(Conversion.Int(curValue/24));
		...
		static string GetOSUptime()
		{
			try
			{
				...
				do
				{
					intDays = Conversion.Int(intTicks/86400000);
				...
				do
				{
					intHours = Conversion.Int(intRemainder/3600000);
				...
				do
				{
					intMinutes = Conversion.Int(intRemainder/60000);
				...
				do
				{
					intSeconds = Conversion.Int(intRemainder/1000);
		...
		void GetSoundInfo()
		{
			var query = new SelectQuery("Win32_SoundDevice");
			var search = new ManagementObjectSearcher(query);
			int count = 0;
		...
		public bool UserIsAdministrator
		{
			get
			{
				mvarUserIsAdministrator = GetUserIsAdministrator();
		...
		public string VidPrimaryScreenDimensions
		{
			get
			{
				mvarVidPrimarytScreenDimensions = GetVidPrimaryScreenDimensions();
		...
		public string VidPrimaryScreenWorkingArea
		{
			get
			{
				mvarVidPrimaryScreenWorkingArea = GetVidPrimaryScreenWorkingArea();
