...
namespace FileEraser
{
	...
	public partial class FormMain : Form
	{
		...
		void frmMain_Load(object sender, EventArgs e)
		{
			...
			// set up the events
			//oShredder.ProgressMax += evtProgressMax;
			//oShredder.ProgressTick += evtProgressTick;
			//oShredder.Complete += evtComplete;
		...
		void DeleteFile(string filePath)
		{
			...
			//stInfo.Items[0].Text = rm.GetString("processing")+" " + filePath;
			Application.DoEvents();
			oShredder.AnyAttribute = true;
			oShredder.DeleteDirectory = false;
			//oShredder.DeleteDirectory = (optFolder.Checked);
			...
			{
				//stInfo.Items[0].Text = rm.GetString("file_destroyed")+"!";
				deletedFilesCount++;
				return;
			}
			//stInfo.Items[0].Text = rm.GetString("deletion_of")+" " + filePath + " "+rm.GetString("failed")+"!";
			return;
		}
