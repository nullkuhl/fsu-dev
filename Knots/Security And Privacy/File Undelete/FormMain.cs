...
namespace FileUndelete
{
	...
	public partial class FormMain : Form
	{
		...
		static bool PathMatch(string pattern, string value)
		{
			try
			{
		...
		bool ItemFound(string filePath, ulong fileId, bool flag, uint size)
		{
			...
			bool add = string.IsNullOrEmpty(filterPath) || PathMatch(filterPath.ToLower(), filePath.ToLower());
			...
			{
				add = Regex.IsMatch(filePath.ToLower(), WildcardToRegex(filterName.ToLower()));
			}
			if (add && isFilterSize)
			{
				add = (size < filterSize && filterSizeSmall) ||
				      (size >= filterSize && !filterSizeSmall);
			}
			if (add)
			{
				add = flag || filterLost;
		...
		void btnScan_Click(object sender, EventArgs e)
		{
			...
			//drivesCombo.SelectedIndex < Drives.Count ? Drives[drivesCombo.SelectedIndex].Name :
			//drivesCombo.SelectedItem.ToString();
			SwitchMode(true);
		...
		void UpdateExtensionTree()
		{
			...
			{
				var node = new TreeNode(item.Key.ToString()) {Name = item.Key.ToString()};
				node.ImageIndex =
		...
		void updateNode(string data, TreeNode parent, bool root)
		{
			...
			if (Node == null)
			{
				...
				Node.Name = Node.Text;
				Node.ImageIndex = Node.SelectedImageIndex = iconListManager.AddFileIcon("");
		...
		void OnFinish()
		{
			...
			lvFiles.BeginUpdate();
			trvFolders.Nodes.Clear();
			trvFolders.Nodes.Add("", cboDrives.SelectedIndex < drives.Count
		...
		{
			var Item = new List<ListViewItem>();
			ShowProcessing();
		...
		void DisplayFiles(string path, string extension)
		{
			...
			foreach (FileToRestore file in filesToRestore)
			{
				...
				if (isFilterName && !string.IsNullOrEmpty(filterName))
				{
					Add = Regex.IsMatch(file.FilePath, WildcardToRegex(filterName));
				}
				if (Add && isFilterSize)
				{
