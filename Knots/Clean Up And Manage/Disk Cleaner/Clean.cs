...
namespace Disk_Cleaner
{
	...
	public static class Clean
	{
		...
		public static ulong ProcessJunk(ArrayList junk)
		{
			...
			var success = new Dictionary<DeleteFile, string>();
		...
		public static void RestoreFolder(string folder, bool overwrite)
		{
			...
			foreach (XmlNode node in nodes)
			{
				...
				if (element != null) from = restorefrom + element.InnerText;
				XmlElement xmlElement = node["filename"];
				string to = null;
				if (xmlElement != null) to = xmlElement.InnerText;
				if (!File.Exists(from))
				...
				else
				{
					...
					else
					{
						try
						{
							if (from != null) if (to != null) File.Copy(from, to, overwrite);
							succ = "OK";
						}
						catch
						{
			...
			foreach (XmlNode node in result.Keys)
				if (result[node] == "OK")
				{
					if (xml.DocumentElement != null) xml.DocumentElement.RemoveChild(node);
					try
					{
						var xmlElement = node["guid"];
						if (xmlElement != null) File.Delete(restorefrom + xmlElement.InnerText);
					}
					catch
					{
					}
