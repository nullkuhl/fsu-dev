...
namespace EmptyFolderFinder
{
	...
	{
		//Folders to exclude
		readonly ArrayList excludeFolder = new ArrayList();
		...
		public FormEmptyFolderFinder()
		{
			...
			//// Simple CustomDialog test
			//CustomDialog msgbox = new CustomDialog(rm.GetString("error_deleting_file"), rm.GetString("error_deleting_file"), "ex.Message", SystemIcons.Exclamation.ToBitmap());
			//msgbox.ShowDialog();
		}
		...
		public void OpenCustomFileDialog()
		{
			...
			var customDialogbox = new frmCustomFileDialog();
			...
			{
				if (customDialogbox.PathToScan == "" || !Directory.Exists(customDialogbox.PathToScan))
				{
		...
		public void FindEmptyFoldersMethod(string sPathtoScan)
		{
            ...
            try
            {
                if (sPathtoScan != "" || sPathtoScan != String.Empty)
                {
                    //if (!bMultipleLocation)
                    //{
                    ...
                    //if (IS_CLEAR_WHOLE_LIST == 0 && !ABORT)
                    //{
                    ...
                    //    DirectoryInfo di = new DirectoryInfo(sPathtoScan);

                    //    DirectoryInfo[] directories = di.GetDirectories();
                    //    emptyListArray = new ArrayList();
                    //    if (!bMultipleLocation)
                    //    {
                    //        //_EmptyFoldersCollection.Clear();
                    //    }
                    ...
                    //            DirectoryInfo[] dires = dInfo.GetDirectories();
                    //            if (files.Length == 0 && dires.Length == 0)
                    ...
                    //if (emptyListArray.Count > 0)
                    //{
                    //    for (int i = 0; i < emptyListArray.Count; i++)
                    //    {
                    //        ListViewItem lstViewItem = new ListViewItem(((System.IO.DirectoryInfo)((emptyListArray)[i])).Name);
                    //        lstViewItem.SubItems.Add(((System.IO.DirectoryInfo)((emptyListArray)[i])).FullName.ToString());
                    //        lstViewItem.SubItems.Add(((System.IO.DirectoryInfo)((emptyListArray)[i])).FullName.ToString());
		...
		public void Scan(string path)
		{
			...
			try //handling unaccessable folders
			{
				...
				if (dInfo.GetFiles().Length == 0 && dInfo.GetDirectories().Length == 0)
				{
					...
					emptyListArray.Add(dInfo);
					var lstViewItem = new ListViewItem(dInfo.Name) { ImageIndex = 1 };
					bool ishap = false;
					foreach (string str in excludeFolder)
					{
						if (str == dInfo.FullName)
						{
							ishap = true;
						}
					}
					...
					//   lstViewItem.SubItems.Add(dInfo.FullName.ToString());
					//ListBoxItem item = new ListBoxItem();
					//item.Content = new Image() { Source = thumbnail };
				...
				foreach (DirectoryInfo subFolder in dInfo.GetDirectories())
				{
					...
					foreach (object t in excludeFolder)
					{
						if (t.ToString() == subFolder.FullName)
						{
							isExclude = true;
						}
					}
		...
		void FrmEmptyFolderFinder_Load(object sender, EventArgs e)
		{
			...
			//this.listView1.ListViewItemSorter = new Sorter();
		}
		...
		void tsbDelete_Click(object sender, EventArgs e)
		{
			...
			while (lvMain.CheckedIndices.Count > 0)
			{
				try
				{
					...
					var dirinfo = new DirectoryInfo(lvMain.Items[checkedIndex].SubItems[1].Text);
					dirinfo.Delete();
		...
		void tsbExclude_Click(object sender, EventArgs e)
		{
			try
			{
				if (lvMain.Items.Count > 0)
				{
					var sw = new StreamWriter(@"Files\ExcludeFiles.txt", true);
		...
		void SetCulture(CultureInfo culture)
		{
			...
			var rm = new ResourceManager("EmptyFolderFinder.Resources", typeof(FormEmptyFolderFinder).Assembly);
