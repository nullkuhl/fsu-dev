...
namespace Uninstall_Manager
{
	...
	public class InstalledProgram : IComparable<InstalledProgram>, IEquatable<InstalledProgram>
	{
		...
		{
            // Changed based on comment from Andreas Breitschopp 9.12.2012
		...
		{
			return DisplayName == other.DisplayName && Version == other.Version;
		}
		...
		static List<InstalledProgram> InternalGetInstalledPrograms(bool includeUpdates, RegistryKey hklmPath,
																   RegistryKey hkuPath)
		{
			var programList = new List<InstalledProgram>();
																  ...
																  List<InstalledProgram> existingProgramList)
		{
			if ((cuInstallerKey != null))
			{
				foreach (string cuProductGuid in cuInstallerKey.GetSubKeyNames())
				{
					...
					var openSubKey = hklmRootKey.OpenSubKey("Software\\Microsoft\\Windows\\CurrentVersion\\Installer\\UserData");
					if (openSubKey != null)
						foreach (
							string userDataKeyName in
								openSubKey.GetSubKeyNames())
						{
							...
							if (userDataKeyName != "S-1-5-18")
							{
								...
								if ((productsKey != null))
								{
									...
									foreach (string lmProductGuid in lmProductGuids)
									{
										...
										{
											var registryKey = cuInstallerKey.OpenSubKey(cuProductGuid);
											if (registryKey != null)
											{
												var name = Convert.ToString(registryKey.GetValue("ProductName", string.Empty));
				...
				foreach (string subKeyName in uninstallKey.GetSubKeyNames())
				{
					try
					{
						...
						if (isSystemComponent != 1)
						{
							...
							{
								var windowsUpdateRegEx = new Regex("KB[0-9]{6}$");
								...
								else
								{
									...
									if (uninstallStringExists)
									{
										...
										string dInstallSize = "";
										string dtInstallDate = "";
										string sDisplayIconPath = "";
