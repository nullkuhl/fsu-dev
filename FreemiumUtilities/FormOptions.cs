...
namespace FreemiumUtilities
{
	...
	public partial class FormOptions : Form
	{
		...
		void Languages_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			var oneClickAppsViewModel = (OneClickAppsViewModel) Application.Current.MainWindow.DataContext;
			...
			switch (oneClickAppsViewModel.StatusTitleKey)
			{
				...
				default:
					{
						oneClickAppsViewModel.StatusTitle = "";
						break;
					}
			}
		...
		void SetContextMenuRegistryKey(RegistryKey root, string titleNode, string title, string knotCodeName)
		{
			try
			{
				...
				if (contextMenuKey != null)
				{
					...
					RegistryKey registryKey = contextMenuKey.CreateSubKey("command");
					if (registryKey != null)
						registryKey
							.SetValue(
								"",
		...
		void RemoveContextMenuRegistryValue(RegistryKey root, string subkey, string valueName)
		{
			try
			{
				RegistryKey openSubKey = root.OpenSubKey(subkey, true);
				if (openSubKey != null) openSubKey.DeleteValue(valueName);
			}
			catch
			{
			}
