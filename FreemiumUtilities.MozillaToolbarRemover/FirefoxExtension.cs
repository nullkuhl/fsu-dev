...
namespace FreemiumUtilities.MozillaToolbarRemover
{
    ...
    {
        public delegate void ReportProgressEventHandler(int progressValue, string name);
        public static event ReportProgressEventHandler ReportProgressEvent;
        ...
        {
            List<FirefoxExtension> changes = extensions.Where(ex => ex.changeRequired != null).ToList();
            if (ff4Mode)
            {
                int i = 0;
                foreach (FirefoxExtension a in changes)
                {
                    SQLiteCommand command = sqliteConn.CreateCommand();
                    command.CommandText = "UPDATE addon SET userDisabled=" + (a.IsEnabled ? "0" : "1")
                                          + ",active=" + (a.IsEnabled ? "1" : "0") + " WHERE addon.id=='" + a.Id + "'";
                    command.ExecuteNonQuery();
                    int progressPercentage = (int)((double)(i + 1) / changes.Count * 100);
                    ReportProgressEvent(progressPercentage, a.Name);
                }
            }
            else
            {
